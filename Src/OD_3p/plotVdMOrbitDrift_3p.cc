...
//            if (istposTmp >= stTmp.size()) break; 
        ...
        } else if (nLabels > iNlabelStart || stfiline.find(stSTpars[iCurrIdx]) == string::npos) {
            blSPsubparRead[0] = true;
        }
    }
...
void sortDisplData (int NDispVal[2], Double_t*** dbDispVal) {
...
int main(int argc, char **argv) {
    ...
    string stDFcolTitle[Nvars] = {"Timestamp", "Beam 1 (x)", "Beam 1 (y)", "Beam 2 (x)", "Beam 2 (y)"};
    ...
    for (int i=0; i<2; i++) dbYrange[i] = new Double_t [2];
...
//    }
    ...
    if (iPlotTimeRange[1] < lScanTimeWindows[0][0]) {
        lScanTimeZones[0][0] = iPlotTimeRange[1];
    } else {
...
// Make left and right zone indents similar
    int iMinLZ, iLZtmp;
...
// Convert Scans time from Timestamp to minutes
    ...
    int NvarValBPM[3] = {0, 0, 0};  // [BPM 1 / BPM 2 / Nominal]
    Double_t*** dbVarInBPM = new Double_t** [3];  // [BPM 1 / BPM 2 / Nominal]
    for (int j=0; j<3; j++) {
        dbVarInBPM[j] = new Double_t* [Nvars];  //timestamp, B1_H, B1_V, B2_H, B2_V
        for (int i=0; i<Nvars; i++) dbVarInBPM[j][i] = new Double_t [NdataMax];
    }
  
    readDataFileNom (stDataFileNom, Nvars, iDFcolIdxNom, iPlotTimeRange, NvarValBPM[2], dbVarInBPM[2]);
  
    for (int i=0; i<Nvars; i++) {
        for (int j=0; j<NvarValBPM[2]; j++) {
            dbVarInBPM[2][i][j] = dbVarInBPM[2][i][j] * dbDFcolScaleNom[i];
        }
    }
...
//calculate Orbit Drift parameters in minutes
  ...
  */
    ...
    string stDataFileBPM[2];  // data file bpm 1 / bpm 2
    string stDataFileTitleBPM[3] = {string(""), string(""), string("Nominal")};
    int iNoffBPM[3] = {0, 0, 0};
    long lDFofftimeBPM[3][NoffMax];
    ...
    string stLegBPM[3] = {string(""), string(""), string("Nominal")};
    for (int k=0; k<2; k++) {
        if (stDataFileTitleBPM[k] == string("arcBPM")) {    // not a clean solution
            stLegBPM[k] = string("Arc");
        } else {
            stLegBPM[k] = stDataFileTitleBPM[k];
        }
    }
...
//prepare BPM1 and BPM2 for Nom < 'Y cut'
    ...
    Double_t**** dbVarBPMcut = new Double_t*** [2];  //[BMP1, BPM2]
    for (int k=0; k<2; k++) {
        dbVarBPMcut[k] = new Double_t** [Nvars-1];  //[B1_H, B1_V, B2_H, B2_V]
        for (int j=0; j<(Nvars-1); j++) {
            dbVarBPMcut[k][j] = new Double_t* [2];  //[Time, Val]
            for (int i=0; i<2; i++) {
                dbVarBPMcut[k][j][i] = new Double_t [NdataMax];  //[data point]
            }
        }
    }
    ...
    int NBPMDispCut[2] = {0, 0};
    string stODXY[2] = {"X", "Y"};    // [X, Y]
    //Double_t dbBPMDispCut[2][2][2][NdataMax]; // [X, Y] [BMP1, BPM2] [Time, Val] [data point]
    Double_t**** dbBPMDispCut = new Double_t*** [2];  //[X, Y]
    for (int k=0; k<2; k++) {
        dbBPMDispCut[k] = new Double_t** [2];  //[BMP1, BPM2]
        for (int j=0; j<2; j++) {
            dbBPMDispCut[k][j] = new Double_t* [2];  //[Time, Val]
            for (int i=0; i<2; i++) {
                dbBPMDispCut[k][j][i] = new Double_t [NdataMax];  //[data point]
            }
        }
    }
    
    bool blTmp;
    Double_t dbDispXtmp, dbDispYtmp;
    double dbdT = 0.0000001; //small time window
  
    for (int jn=0; jn<NvarValBPM[2]; jn++) {
        if (jn==0) {
            dbTnomRange[0] = dbVarInBPM[2][0][jn];
        } else {
            dbTnomRange[0] = (dbVarInBPM[2][0][jn-1] + dbVarInBPM[2][0][jn])/2;
        }
        if (jn==(NvarValBPM[2] - 1)) {
            dbTnomRange[1] = dbVarInBPM[2][0][jn];
        } else {
            dbTnomRange[1] = (dbVarInBPM[2][0][jn+1] + dbVarInBPM[2][0][jn])/2;
        }
        ...
        for (int k=0; k<2; k++) {
            for (int j=iBPMstart[k]; j<NvarValBPM[k]; j++) {
                if (dbVarInBPM[k][0][j] >= dbTnomRange[1]) {
                    ...
                    for (int i=0; i<4; i++) {
                        if (dbVarInBPM[2][i+1][jn]*dbVarInBPM[2][i+1][jn] < dbNomYcut*dbNomYcut) {
...
// Evaluate relative displacement for BPM1 and BPM2
                    ...
                    for (int i=0; i<4; i++) blTmp = blTmp && (dbVarInBPM[2][i+1][jn]*dbVarInBPM[2][i+1][jn] < dbNomYcut*dbNomYcut) && (jn == 0 || dbVarInBPM[2][i+1][jn-1]*dbVarInBPM[2][i+1][jn-1] < dbNomYcut*dbNomYcut) && (jn == (NvarValBPM[2] - 1) || dbVarInBPM[2][i+1][jn+1]*dbVarInBPM[2][i+1][jn+1] < dbNomYcut*dbNomYcut);
                    if (blTmp) {
    ...
    } //end loop over Nominal Nvars

    for (int i=0; i<2; i++) sortDisplData(NBPMDispCut, dbBPMDispCut[i]);
  
// Evaluate the Orbit Drifts
    double dbODBPMaveVal[2][2][NscansMax][3];   // result of the average of BPM displacements [X, Y] [BPM1/2] [scan N] [0 - beginning, 1 - middle, 2 - end]
    ...
    double dbODextrVal[2][2][NscansMax][3];     // Orbit Drifts values lineary extrapolated to scan beginning / end [X, Y] [0 - (BPM1 - Nom), 1 - (BPM2 - Nom)] [scan N] [0 - beginning, 1 - middle, 2 - end]
    ...
    for (int k=0; k<2; k++) {    // BPM 1/2
        ...
        // Evaluate Orbit Drifts before the Scans
            ...
            for (int iDv=(NBPMDispCut[k]-1); iDv>=0; iDv--) {
                if (!blInODtw) {
                    if (dbBPMDispCut[0][k][0][iDv] < (dbScanTimeWindows[iSc][0] - dbODpars[0])) {
                        if (dbBPMDispCut[0][k][0][iDv] < dbScanTimeZones[iSc][0]) {
//                          cout << "ERROR: latest BPM candidate time value before the scan " << stScanNames[iSc] << " is outside scan zone:" << endl;
//                          cout << "     BPM t: " << dbBPMDispCut[0][k][0][iDv] << " < " << dbScanTimeZones[iSc][0] << "." << endl;
                            break;
                        } else {
                            dbODBPMaveTime[k][iSc][0][1] = dbBPMDispCut[0][k][0][iDv] + dbdT;
                            blInODtw = true;
                        }
                    }
                }
                if (blInODtw) {
                    if (dbBPMDispCut[0][k][0][iDv] > dbScanTimeZones[iSc][0] && dbBPMDispCut[0][k][0][iDv] > (dbODBPMaveTime[k][iSc][0][1] - dbdT - dbODpars[1])) {
                        for (int iXY=0; iXY<2; iXY++) dbODBPMaveVal[iXY][k][iSc][0] += dbBPMDispCut[iXY][k][1][iDv];
                        iNODaveTmp++;
                        dbODBPMaveTime[k][iSc][0][0] = dbBPMDispCut[0][k][0][iDv] - dbdT;
            
                        if (iDv == 0) {
                            for (int iXY=0; iXY<2; iXY++) dbODBPMaveVal[iXY][k][iSc][0] = dbODBPMaveVal[iXY][k][iSc][0] / iNODaveTmp;
                            blInODtw = false;
                            break;
                        }          
                    } else {
        ...
        // Evaluate Orbit Drifts in the middle of the Scans
            iNODaveTmp = 0;
            for (int iXY=0; iXY<2; iXY++) dbODBPMaveVal[iXY][k][iSc][1] = 0;
            ...
            for (int iDv=0; iDv<NBPMDispCut[k]; iDv++) {
                if (!blInODtw) {
                    if (dbBPMDispCut[0][k][0][iDv] > (dbScanTimeWindows[iSc][0] + dbODpars[0])) {
                        if (dbBPMDispCut[0][k][0][iDv] > dbScanTimeZones[iSc][1]) {
//                            cout << "ERROR: first BPM candidate time value after the scan start " << stScanNames[iSc] << " is outside scan zone:" << endl;
//                            cout << "     BPM t: " << dbBPMDispCut[0][k][0][iDv] << " > " << dbScanTimeZones[iSc][1] << "." << endl;
                        }
                        dbODBPMaveTime[k][iSc][1][0] = dbBPMDispCut[0][k][0][iDv] - dbdT;
                        blInODtw = true;
                    }
                }
                if (blInODtw) {
                    blTmp = true;
//                    if (dbBPMDispCut[0][k][0][iDv] > (dbScanTimeWindows[iSc][0] + dbODpars[0])) blTmp = dbBPMDispCut[0][k][0][iDv] < (dbODBPMaveTime[k][iSc][1][0] + dbdT + dbODpars[1]);
                    if (dbODBPMaveTime[k][iSc][1][0] < dbScanTimeWindows[iSc][1] - dbODpars[0]) {
                        if (dbBPMDispCut[0][k][0][iDv] > dbScanTimeWindows[iSc][1] - dbODpars[0]) blTmp = false;
                    } else {
                        if (dbBPMDispCut[0][k][0][iDv] > dbScanTimeZones[iSc][1] - dbODpars[0]) blTmp = false;
                    }
                    if (blTmp) {
                        for (int iXY=0; iXY<2; iXY++) dbODBPMaveVal[iXY][k][iSc][1] += dbBPMDispCut[iXY][k][1][iDv];
                        iNODaveTmp++;
                        dbODBPMaveTime[k][iSc][1][1] = dbBPMDispCut[0][k][0][iDv] + dbdT;
            
                        if (iDv == (NBPMDispCut[k]-1)) {
                            for (int iXY=0; iXY<2; iXY++) dbODBPMaveVal[iXY][k][iSc][1] = dbODBPMaveVal[iXY][k][iSc][1] / iNODaveTmp;
                            blInODtw = false;
                            break;  
                        }
                    } else {
        ...
        // Evaluate Orbit Drifts after the Scans
            iNODaveTmp = 0;
            for (int iXY=0; iXY<2; iXY++) dbODBPMaveVal[iXY][k][iSc][2] = 0;
            dbODBPMaveTime[k][iSc][2][0] = 0; dbODBPMaveTime[k][iSc][2][1] = 0;
            //Search for the first BPM candidate value after the scan
            blInODtw = false;
            for (int iDv=0; iDv<NBPMDispCut[k]; iDv++) {
                if (!blInODtw) {
                    if (dbBPMDispCut[0][k][0][iDv] > (dbScanTimeWindows[iSc][1] + dbODpars[0])) {
                        if (dbBPMDispCut[0][k][0][iDv] > dbScanTimeZones[iSc][1]) {
//                            cout << "ERROR: first BPM candidate time value after the scan " << stScanNames[iSc] << " is outside scan zone:" << endl;
//                            cout << "     BPM t: " << dbBPMDispCut[0][k][0][iDv] << " > " << dbScanTimeZones[iSc][1] << "." << endl;
                        }
                        dbODBPMaveTime[k][iSc][2][0] = dbBPMDispCut[0][k][0][iDv] - dbdT;
                        blInODtw = true;
                    }
                }
                if (blInODtw) {
                    if (dbBPMDispCut[0][k][0][iDv] < (dbODBPMaveTime[k][iSc][2][0] + dbdT + dbODpars[1])) {
                        for (int iXY=0; iXY<2; iXY++) dbODBPMaveVal[iXY][k][iSc][2] += dbBPMDispCut[iXY][k][1][iDv];
                        iNODaveTmp++;
                        dbODBPMaveTime[k][iSc][2][1] = dbBPMDispCut[0][k][0][iDv] + dbdT;
            
                        if (iDv == (NBPMDispCut[k]-1)) {
                            for (int iXY=0; iXY<2; iXY++) dbODBPMaveVal[iXY][k][iSc][2] = dbODBPMaveVal[iXY][k][iSc][2] / iNODaveTmp;
                            blInODtw = false;
                            break;  
                        }
                    } else {
    ...
    double dbODoutVal[2][4][NscansMax][3];      // Orbit Drifts values to save / end [X, Y] [0 - (BPM1 - Nom), 1 - (BPM2 - Nom), 2 - (BPM2 - BPM1), 3 - (BPM2 + BPM1)/2] [scan N] [0 - beginning, 1 - middle, 2 - end]
    double dbODoutTime[4][NscansMax][3];        // Orbit Drifts times to save / end [0 - (BPM1 - Nom), 1 - (BPM2 - Nom), 2 - (BPM2 - BPM1), 3 - (BPM2 + BPM1)/2] [scan N] [0 - beginning, 1 - middle, 2 - end]
  ...
  // Prepare Orbit Drifts for saving
    for (int iSc=0; iSc<iNscans; iSc++) {
        for (int j=0; j<3; j++) {
            ...
            for (int iXY=0; iXY<2; iXY++) {
                for (int k=0; k<2; k++) dbODoutVal[iXY][k][iSc][j] = dbODBPMaveVal[iXY][k][iSc][j];
...
// write orbit drifts to the files
  ...
  for (int k=0; k<4; k++) {
    ...
    stFoutName = stFoutName + string(".json");
...
// double(<timestamp> - iPlotTimeRange[0]) )*dbDFcolScaleNom[0]
    ...
    TCanvas *canvasBPM = new TCanvas("canvasBPM", "canvasBPM",    3400, 1050);      // canvas for the BPM plots
  
    TPad *pad[5];
    pad[0] = new TPad( "pad1", "OD X pad", 0.01, 0.46, 0.99, 0.91 );
    pad[1] = new TPad( "pad2", "OD Y pad", 0.01, 0.01, 0.99, 0.46 );
    pad[2] = new TPad( "pad3", "BPM1 pad", 0.01, 0.01, 0.99, 0.87 );
    pad[3] = new TPad( "pad4", "BPM2 pad", 0.01, 0.01, 0.99, 0.87 );
    pad[4] = new TPad( "pad5", "Nom  pad", 0.01, 0.01, 0.99, 0.87 );
    
    double p_margin_T = 0.05;
    double p_margin_B = 0.145;
    double p_margin_L = 0.06;
    double p_margin_R = 0.16;
    
    for (int ip=0; ip<5; ip++) {
        pad[ip]->SetTicks(1,1);
        pad[ip]->SetTopMargin(p_margin_T);
        pad[ip]->SetBottomMargin(p_margin_B);
        pad[ip]->SetLeftMargin(p_margin_L);
        pad[ip]->SetRightMargin(p_margin_R);
    }
    
    // change OD top and bottom plots margins
    double ch_margin = ( (p_margin_T < p_margin_B) ? p_margin_T : p_margin_B ) * 0.60;
    pad[0]->SetBottomMargin(p_margin_B - ch_margin);
    pad[1]->SetTopMargin(p_margin_T - ch_margin);
  
    canvasmain->cd();
    for (int ip=0; ip<2; ip++) pad[ip]->Draw();
  
    TGraph *tgVdmBPM[3][Nvars - 1];   // [BPM 1 / BPM 2 / Nominal] [variable]
    TGraph *tgBPMDisp[2][4];      // [X, Y] [BPM1 / BPM2 / copy BPM1 / copy BPM2]
  
    TLine *tlODaveLine[2][2][iNscans][3];  // Orbit Drift average line [X, Y] [BPM1 / BPM2] [Scan num] [begin / middle / end] 
    TLine *tlODconnectLine[2][2][iNscans][2]; // [X, Y] [BPM1 / BPM2] [Scan num] [begin / end]
    ...
    string stPlotYLeg[5]  = {"w [#mu m]", "h [#mu m]", "w, h [#mu m]", "w, h [#mu m]", "w, h [#mu m]"};
    double dbPlotYLegX[5] = {0.027,          0.027,           0.020,          0.020,          0.020};

    double dbLegSep = 0.06;
    TLegend* tLeg;
    TLatex tLabel;
...
//prepare graph for displacement
  ...
  // remove X axis labels for the top OD plot
    tgBPMDisp[0][0]->GetXaxis()->SetLabelOffset(999);
    tgBPMDisp[0][0]->GetXaxis()->SetLabelSize(0);
...
/*  
        tLeg = new TLegend(0.850,(0.5 - (p_margin_T/2) + (p_margin_B/2) - NlegEntr*dbLegSep),0.997,(0.5 - (p_margin_T/2) + (p_margin_B/2) + NlegEntr*dbLegSep));
        
        tLeg->SetTextFont(42);
        tLeg->SetTextSize(0.08);
        for (int k=0; k<2; k++) {
            tLeg->AddEntry(tgBPMDisp[iXY][k+2],stLegBPM[k].c_str(),"p");
        }
...
*/
        ...
        if (iXY != 0) tLabel.DrawLatex(1 - p_margin_R,0.025,"t [min]");

        tLabel.SetTextAngle(90);        
        tLabel.SetTextAlign(31);
        tLabel.DrawLatex(dbPlotYLegX[iXY], 1 - p_margin_T, stPlotYLeg[iXY].c_str());
    ...
    // draw legend in canvas
    {
        canvasmain->cd();
        tLeg = new TLegend(0.850,(0.5 - ((p_margin_T/2) + (p_margin_B/2) - NlegEntr*dbLegSep) / 2),0.997,(0.5 - ((p_margin_T/2) + (p_margin_B/2) + NlegEntr*dbLegSep)/2));
        tLeg->SetTextFont(42);
        tLeg->SetTextSize(0.04);
        for (int k=0; k<2; k++) {
            tLeg->AddEntry(tgBPMDisp[0][k+2],stLegBPM[k].c_str(),"p");
        }
        tLeg->SetTextAlign(12);
        tLeg->SetBorderSize(0);
        tLeg->Draw();
    ...
    if (stPlotTitle[0] != string("None")) {
        ...
        tLabel.DrawLatex(0.472, 0.93, stPlotTitleFin.c_str());
    }
    
    tLabel.SetTextFont(42);
    tLabel.SetTextSize(0.75 * p_margin_T*0.45 * 1.9);
    if (stPlotTitle[1] != string("None")) {
        tLabel.SetTextAlign(11);
        tLabel.DrawLatex(0.01 + p_margin_L*0.98, 1 - 0.09 - p_margin_T*0.45 + 0.6*p_margin_T*0.45, stPlotTitle[1].c_str());
    }
    if (stPlotTitle[2] != string("None")) {
        tLabel.SetTextAlign(31);
        tLabel.DrawLatex(0.99 - p_margin_R*0.98, 1 - 0.09 - p_margin_T*0.45 + 0.6*p_margin_T*0.45, stPlotTitle[2].c_str());
    }

    
    canvasmain->Print( string(stPlotFile + "OrbitDrift_XY.eps").c_str(), "eps");
//    canvasmain->SaveAs( string(stPlotFile + "OrbitDrift_XY.root").c_str() );
  
// Draw BPM1 / BPM2 / Nominal:
    for (int ibpm = 0; ibpm < 3; ibpm++) {
        ...
        for (int i=0; i<(Nvars - 1); i++) {
            ...
            tgVdmBPM[ibpm][i]->GetXaxis()->SetLabelSize(0.06);
            tgVdmBPM[ibpm][i]->GetYaxis()->SetLabelSize(0.06);
        ...
        tLeg = new TLegend(0.860,(0.5 - (p_margin_T/2) + (p_margin_B/2) - NlegEntr*dbLegSep),0.997,(0.5 - (p_margin_T/2) + (p_margin_B/2) + NlegEntr*dbLegSep));
        tLeg->SetTextFont(42);
        tLeg->SetTextSize(0.07);
        for (int i=0; i<(Nvars - 1); i++) {
            tLeg->AddEntry(tgVdmBPM[ibpm][i],stDFcolTitle[i+1].c_str(),"l");
        }
        tLeg->SetTextAlign(12);
        tLeg->SetBorderSize(0);
        tLeg->Draw();
        ...
        tLabel.SetTextSize(0.076);
        tLabel.SetTextAlign(31);
        tLabel.DrawLatex(1 - p_margin_R,0.025,"t [min]");
        
        tLabel.SetTextAngle(90);
        tLabel.SetTextAlign(31);
        tLabel.DrawLatex(dbPlotYLegX[ibpm + 2], 1 - p_margin_T, stPlotYLeg[ibpm + 2].c_str());
        ...
        if (stPlotTitle[0] != string("None")) {
            ...
            tLabel.DrawLatex(0.472, 0.890, stPlotTitleFin.c_str());
        }
        
        tLabel.SetTextFont(42);
        tLabel.SetTextSize(0.75 * p_margin_T*0.86 * 1.9);
        if (stPlotTitle[1] != string("None")) {
            tLabel.SetTextAlign(11);
            tLabel.DrawLatex(0.01 + p_margin_L*0.98, 1 - 0.13 - p_margin_T*0.86 + 0.6*p_margin_T*0.86, stPlotTitle[1].c_str());
        }
        
        if (stPlotTitle[2] != string("None")) {
            tLabel.SetTextAlign(31);
            tLabel.DrawLatex(0.99 - p_margin_R*0.98, 1 - 0.13 - p_margin_T*0.86 + 0.6*p_margin_T*0.86, stPlotTitle[2].c_str());
        }

        canvasBPM->Print( string(stPlotFile + stDataFileTitleBPM[ibpm] + ".eps").c_str(), "eps");
//        canvasBPM->SaveAs(string(stPlotFile + stDataFileTitleBPM[ibpm] +  ".root").c_str() );
    }
...
// remove pointers
    ...
    for (int k=0; k<2; k++) {
        for (int j=0; j<(Nvars-1); j++) {
            for (int i=0; i<2; i++) {
                delete [] dbVarBPMcut[k][j][i];
            }
            delete [] dbVarBPMcut[k][j];
        }
        delete [] dbVarBPMcut[k];
    }
    delete [] dbVarBPMcut;
    ...
    for (int k=0; k<2; k++) {
        for (int j=0; j<2; j++) {
            for (int i=0; i<2; i++) {
                delete [] dbBPMDispCut[k][j][i];
            }
            delete [] dbBPMDispCut[k][j];
        }
        delete [] dbBPMDispCut[k];
    }
    delete [] dbBPMDispCut;
