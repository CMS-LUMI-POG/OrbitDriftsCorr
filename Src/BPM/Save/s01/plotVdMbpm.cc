...
// Evaluation of the new offsets.
  ...
  bool blNewOffSet[NoffMax];
    ...
    for (int i=0; i<(NvarValNom-2); i++) {
        
        for (int j=0; j<4; j++) {
            if ( (dbVarInNom[j+1][i]*dbVarInNom[j+1][i] < dbNomYcut*dbNomYcut) && (dbVarInNom[j+1][i+2]*dbVarInNom[j+1][i+2] < dbNomYcut*dbNomYcut) ) {
                blNomAtZero[j] = true;
            } else {
                blNomAtZero[j] = false;
            }
        ...
        for (int k=iBPMstart; k<NvarValBPM; k++) {
            if (dbVarInBPM[0][k] >= dbVarInNom[0][i+1]) {
                iBPMstart = k;
                break;
            } else if (dbVarInBPM[0][k] >= dbVarInNom[0][i] && dbVarInBPM[0][k] < dbVarInNom[0][i+1]) {
                for (int j=0; j<4; j++) {
                    if (blAllNomAtZero) {
